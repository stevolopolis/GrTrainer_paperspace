...
def compare_reps(samples_per_cls=20, n_samples=10, model_name='alexnetMap'):
    ...
    for i in range(n_samples):
        ...
        cls_baseline_rsm, cls_baseline_activations, _ = get_RSM(cls_baseline_model, samples_per_cls=samples_per_cls, model_type='alexnet', seed=seed)
        ...
        print(np.mean(cls_cls_corr), np.mean(grasp_cls_corr))
        input()
        print(np.mean(cls_grasp_corr), np.mean(grasp_grasp_corr))
        input()
