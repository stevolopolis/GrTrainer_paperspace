...
scheduler = torch.optim.lr_scheduler.StepLR(optim, 50, 0.5)
...
for epoch in tqdm(range(1, params.EPOCHS + 1)):
    ...
    #for step, (img, cls_map, label) in enumerate(data_loader.load_batch()):
    # Data loop for Grasp training
    for step, (img, grasp_map) in enumerate(data_loader.load_grasp_batch()):
        optim.zero_grad()
        output = model(img)

        # Loss fn for CLS training
        #loss = MapLoss(output, cls_map)
        # Loss fn for Grasp training
        loss = MapLoss(output, grasp_map)
